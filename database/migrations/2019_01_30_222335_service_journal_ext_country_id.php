...
class ServiceJournalExtCountryId extends Migration
{
    /**
     * Run the migrations.
     *
    ...
    public function up()
    {
        Schema::table('service_journal', function (Blueprint $table){
            ...
            $table->foreign('country_id','service_journal_country_fk')->references('id')->on('country');
        });
        ...
        DB::statement("
            update service_journal sj
              ...
              on s.id = sj.service_id
                inner join service_thematic_group stg 
                on stg.id = s.service_thematic_group_id
                  inner join service_category sc 
                  on sc.id = stg.service_category_id
            set 
              sj.country_id = sc.country_id
        ");
