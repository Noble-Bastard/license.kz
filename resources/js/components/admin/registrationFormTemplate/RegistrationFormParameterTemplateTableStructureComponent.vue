            ...
            <div class="col-12">
                ...
                <div>
                    <table id="optionsetList" class="table table-striped table-responsive-sm col-12">
                        <thead>
                        <tr>
                            <th>{{trans.get('messages.admin.registrationFormParameterTemplateTableStructure.id')}}</th>
                            <th>{{trans.get('messages.admin.registrationFormParameterTemplateTableStructure.caption')}}</th>
                            <th>{{trans.get('messages.admin.registrationFormParameterTemplateTableStructure.parameter_type_name')}}</th>
                            <th>{{trans.get('messages.admin.registrationFormParameterTemplateTableStructure.optionset_type_name')}}</th>
                            <th>{{trans.get('messages.admin.registrationFormParameterTemplateTableStructure.is_active')}}</th>
                ...
                <div class="modal-content">
                    ...
                    <div class="modal-body" @change="form.errors.clear($event.target.name)"
                        ...
                        <div class="form-group">
                            <label for="caption">{{trans.get('messages.admin.registrationFormParameterTemplateTableStructure.caption')}}</label>
                        ...
                        <div class="form-group">
                            <div class="custom-control custom-checkbox">
                                <input type="checkbox"
                                       ...
                                       :id="'table_is_active_' + form.id"
                                       v-model="form.is_active">
                                <label class="custom-control-label" :for="'table_is_active_' + form.id">{{trans.get('messages.admin.registrationFormParameterTemplateTableStructure.is_active')}}</label>
                            </div>
                        ...
                        <div class="form-group">
                            <label for="parameter_type_id">{{trans.get('messages.admin.registrationFormParameterTemplateTableStructure.parameter_type_name')}}</label>
                            ...
                            <select :class="['form-control', form.errors.has('parameter_type_id') ? 'is-invalid' : '']"
                                ...
                                <option v-if="value.id != cnstTableParameter" v-for="(value, key) in parameterTypeList" :value="value.id">
                                    {{value.name}}
                                </option>
                            </select>
                        ...
                        <div v-if="form.parameter_type_id == cnstOptionsetParameter" class="form-group">
                            <label for="optionset_type_id">{{trans.get('messages.admin.registrationFormParameterTemplateTableStructure.optionset_type_name')}}</label>
...
<script>
    ...
    export default {
        name: "RegistrationFormParameterTemplateTableStructureComponent",
        data() {
            return {
                tableStructureUrl: '/admin/vue/rfpt_table_structure',
                entityList: null,
                defaultEntity: {
                    id: null,
                    registration_form_parameter_template_table_id: null,
                    column_parameter_template_id: null,
                    parameter_type_id: null,
                    optionset_type_id: null,
                    caption: "",
                    is_active: true,
                    order_number: null,
                    tableParameterId: this.initialTableParameterId
                },
        ...
        props: {
            'initialTableParameterId': Number,
        ...
        computed: {
            modalFormName: function(){
                return ('set-registration-form-parameter-template-table-modal-' + this.initialTableParameterId);
            }
        },
        methods: {
            getEntityList() {
                let requestUrl = this.tableStructureUrl + '/list/'+this.initialTableParameterId;
            ...
            set() {
                let self = this;
                let requestUrl = this.tableStructureUrl + "/store";
            ...
            deleteEntity(entityId, index) {
                axios.post(this.tableStructureUrl + "/delete", {
                    entityId: entityId
                })
                    .then(request => {
            ...
            move(entityId, moveType){
                let requestUrl = this.tableStructureUrl
