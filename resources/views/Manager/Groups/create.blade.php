...
<div class="w-full">
    <div class="px-5 py-5" style="padding-left:20px;padding-right:20px;">
        <div class="flex items-center justify-between gap-[10px] mb-[30px]">
            <h1 class="text-[39px] leading-[1] font-normal tracking-[-0.02em] text-text-primary">Создать группу</h1>
        </div>
    </div>
    ...
    <div class="px-5 pb-20" style="padding-left:20px;padding-right:20px;">
        <div class="max-w-md mx-auto">
            <div class="bg-white rounded-lg p-6 shadow-sm border border-border-light">
                {!! Form::open(['url' => route('Manager.groups.store'), 'method' => 'post', 'class' => 'space-y-6']) !!}
                ...
                <div class="flex items-center gap-3 pt-4">
                    ...
                    {!! Form::submit('Создать группу', ['class' => 'inline-flex items-center justify-center px-4 py-2 bg-[#279760] text-white rounded-lg hover:bg-[#1f7a4f] transition-colors']) !!}
                </div>
                {!! Form::close() !!}
            </div>
        </div>
