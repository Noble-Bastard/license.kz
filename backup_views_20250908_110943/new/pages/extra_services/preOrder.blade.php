        ...
        <div class="row justify-content-center">
          <div class="col-10">
            <form method="post" id="serviceOrderForm" class="form-horizontal"
                  action="{{route('ExtraServices.order')}}">
              @csrf
              <input type="hidden" name="serviceStepIdList" value="{{$serviceStepIdList}}">
              <input type="hidden" name="extraServiceCode" value="{{$extraServiceCode}}">
              <input type="hidden" name="selectedCity" value="{{$selectedCity}}">
              <input type="hidden" name="paymentTypeId"
                     value="{{\App\Data\Helper\PaymentTypeList::TransferPaymentType}}">
